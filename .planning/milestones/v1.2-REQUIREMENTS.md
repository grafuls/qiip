...
- [x] **TEAR-01**: Operator can teardown a node: drain connections, SSH stop container, deregister from etcd
- [x] **TEAR-02**: Force teardown option skips connection drain and immediately stops/deregisters

### Admin API

- [x] **API-01**: POST /admin/nodes/setup accepts hostname, returns 202 with task ID
- [x] **API-02**: GET /admin/provisioning/tasks returns status of all setup/teardown operations
- [x] **API-03**: DELETE /admin/nodes/{id} triggers graceful or forced teardown

### Dashboard

- [x] **DASH-01**: Dashboard has a setup form where operator enters hostname and triggers setup
- [x] **DASH-02**: Each node row has a teardown button
- [x] **DASH-03**: Dashboard displays setup/teardown progress with per-step status
...
| TEAR-01 | Phase 13 | Complete |
| TEAR-02 | Phase 13 | Complete |
